        ...
        overflow: auto;">
            <div class="order_details-nav">
                <div class="list-order-details-lib">
                    <div v-show="details.type == '0' ? true:false ">
                    ...
                    <div class="title-l">备注</div>
                        <div class="text-area order_remark">
                            <textarea name="" id="" cols="30" rows="5" v-model.trim="remark[index]" placeholder="需要开发票，公司抬头：某某某有限公司"></textarea>
                        </div>
                </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            //打开商铺-好友界面
            open_friend() {
                iosObject.toFriendMainPage('friendId');
            },
            submit_e() {
                var that = this;
                let url, _p;  
                    ...
                    _p = {
                        ...
                        // type: that.$route.query.type || null,
                        recive_address: that.address_id,
                        
                        remark: that.remark,
                    };
                if (!that.address_id && that.$route.query.type==0) {
                    that.Toast('请先选择收获地址');
                    return false;
                }
                if (that.viewOrderType == 2) {
